...
def _rand_token(n=20):
    alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
    return "".join(secrets.choice(alphabet) for _ in range(n))

def create_shortlist_link(venue_ids, note=None):
    if not venue_ids:
        return None
    slug  = _rand_slug(8)
    ...
    try:
        db.collection("shortlists").document(slug).set(doc)
    except Exception as e:
        print("[shortlist] Firestore write error:", repr(e))
        return None

    base = os.getenv("CATALOGUE_BASE_URL", "https://evengo.space").rstrip("/")
    return f"{base}/?shortlist={slug}&token={token}"
